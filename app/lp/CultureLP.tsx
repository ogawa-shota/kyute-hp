...
export default function CultureLP(){
 ...
 return <div id="culture-lp">
  ...
  <main id="main">
   ...
   <section className="cs-why cs-section" id="why-kyute"><div className="cs-wrap"><div className="cs-section-opening"><p className="cs-eyebrow">採用の視点を、クリエイティブに。</p><h2>採用を考える。<br/>表現をつくる。<br/>届け続ける。</h2><p>惹かれる理由から、見たくなる企画へ。<br/>KYUTEは、この3つをつなぐチームです。</p></div><div className="cs-disciplines">{[{en:'RECRUITING',jp:'誰に、何が響くのか。',copy:'採用ターゲットを理解し、人・仕事・カルチャーのどこに惹かれるのかを設計する。'},{en:'CREATIVE',jp:'続きが、見たくなる。',copy:'密着、インタビュー、対談。伝えたいことを、見てもらえるコンテンツに変える。'},{en:'OPERATION',jp:'公開を、次のはじまりに。',copy:'視聴データと採用現場の反応から、次の企画を考える。届け方を、継続して磨く。'}].map((item,i)=><div className="cs-discipline" key={item.en}><span className="cs-discipline-index">0{i+1}</span><strong>{item.en}</strong><div><h3>{item.jp}</h3><p>{item.copy}</p></div>{i<2&&<span className="cs-times" aria-hidden="true">×</span>}</div>)}<div className="cs-convergence"><span aria-hidden="true">↓</span><p>カルチャーで、<br className="cs-mobile-break"/><strong>選ばれる採用。</strong></p></div></div></div></section>
   <CultureEngine/>
   <section className="cs-experience cs-section" id="experience"><div className="cs-wrap"><div className="cs-experience-title"><p className="cs-eyebrow">応募数の、その先へ。</p><h2>応募する前に、<br/>志望度を上げる。</h2><p>「条件が合う」から、「ここで働きたい」へ。<br/>お互いを知ったうえで、出会える採用を。</p></div><div className="cs-experience-pair"><div className="cs-experience-before"><p className="cs-experience-label">条件を中心に、会社を知る</p><div className="cs-mini-job"><span>募集要項</span><div>仕事内容 <i/></div><div>給与・待遇 <i/></div><div>勤務地 <i/></div></div><p className="cs-feeling">どんな人と、<br/>働くんだろう。</p><ol><li>会社・求人を知る</li><li>条件を見る</li><li>応募する</li><li>面接でカルチャーを知る</li></ol></div><div className="cs-experience-after"><img src={frames.smile} width="1280" height="720" alt="出演者の自然な表情。映像で人を知る体験の参考" loading="lazy"/><p className="cs-experience-label">カルチャーまで知って、会社を選ぶ</p><p className="cs-feeling">この人たちと、<br/>働いてみたい。</p><ol><li>YouTubeを見る</li><li>人・カルチャーを知る</li><li>ここで働きたい</li><li>応募する</li></ol></div></div><div className="cs-video-data"><div><CountUp value={69}/><p>採用動画を見たいのは、<strong>「応募前」。</strong></p></div><details className="cs-source"><summary>調査の対象・出典を見る <span>＋</span></summary><p>エン転職ユーザー調査、2025年4月。調査全体1,393人／複数回答。69.0%は原典の69%を表示統一したものです。上の比較は採用コミュニケーションの設計例であり、個別企業の採用成果を示すものではありません。</p><a href="https://corp.en-japan.com/newsrelease/2025/41694.html" target="_blank" rel="noopener noreferrer">調査原典を読む ↗</a></details></div></div></section>
  ...
  <footer className="cs-footer"><div><a href="#top" className="cs-logo">KYUTE<span>.</span></a><p>カルチャーを、採用コンテンツに。</p></div><nav aria-label="フッターナビゲーション"><a href="/about">会社概要 <Arrow/></a><a href="/contact">お問い合わせ <Arrow/></a></nav><small>© KYUTE合同会社</small><a className="cs-back-top" href="#top" aria-label="ページの先頭へ">↑</a></footer>
